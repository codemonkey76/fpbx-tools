...
pub fn count_domain_rows(session: &SshSession, domain_uuid: &str) -> Result<DomainTableCounts> {
    ...
    for table in DOMAIN_TABLES {
        ...
        let sql = format!("SELECT COUNT(*) FROM {} WHERE domain_uuid = '{}' 2>/dev/null || echo 0;", table, domain_uuid);
...
pub fn psql_query(session: &SshSession, sql: &str) -> Result<String> {
    ...
    let cmd = format!("sudo -u postgres psql -d fusionpbx -t -A -F'|' -c '{}'", escaped);
    session.exec_ok(&cmd).context("psql_query_failed")
}

fn psql_cmd(sql: &str) -> String {
    let escaped = sql.replace('\'', "'\\''");
    format!(
    "sudo -u postgres psql -d fusionpbx -t -A -F'|' -c '{}' 2>/dev/null || echo 0", escaped)
}
...
impl DomainFilePaths {
    ...
    /// Return only paths that actually exist on the rmeote.
    pub fn existing(&self, session: &SshSession) -> Vec<String> {
        ...
        paths.iter()
            .filter(|p| {
                session.exec_ok(&format!("test -d '{}' && echo yes || echo no", p))
                    .map(|r| r,trim() == "yes")
                    .unwrap_or(false)
            })
            .map(|p| p.to_string())
